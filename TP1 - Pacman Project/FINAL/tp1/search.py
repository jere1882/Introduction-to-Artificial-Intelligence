...
def depthFirstSearch(problem):
    ...
    return search(problem, startStack)

def breadthFirstSearch(problem):
    startQueue = util.Queue()
    return search(problem, startQueue)


def uniformCostSearch(problem):
    "Search the node of least total cost first."
    startPQueue = util.PriorityQueueWithFunction(lambda x : problem.getCostOfActions(x[1])) 
    return search(problem, startPQueue)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    startPQueue = util.PriorityQueueWithFunction(lambda x : g(x) + heuristic(x[0],problem) ) 
    return search(problem, startPQueue)
